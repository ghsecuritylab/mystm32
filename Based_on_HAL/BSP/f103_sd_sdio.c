...
{
	//printf("ready_to_read\r\n");
  if(HAL_SD_ReadBlocks(&uSdHandle, (uint8_t *)pData, ReadAddr, NumOfBlocks, Timeout) != HAL_OK)
  {
		printf("ReadBlocks_ERROR=%d,STATUS=%d\r\n",uSdHandle.ErrorCode,uSdHandle.State);
    return MSD_ERROR;
  }
  else
  {
	  printf("inner_read_ok\r\n");
    return MSD_OK;
  }
}
...
{
	//printf("ready_to_write\r\n");
  if(HAL_SD_WriteBlocks(&uSdHandle, (uint8_t *)pData, WriteAddr, NumOfBlocks, Timeout) != HAL_OK)
  {
		printf("WriteBlocks_ERROR=%d,STATUS=%d\r\n",uSdHandle.ErrorCode,uSdHandle.State);
    return MSD_ERROR;
  }
  else
  {
	  printf("inner_write_ok\r\n");
    return MSD_OK;
  }
}
